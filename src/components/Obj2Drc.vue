...
export default {
  methods: {
    openInputPathDlg() {
      ipcRenderer.send("obj_drc_input")
    },
    openOutputPathDlg() {
      ipcRenderer.send("obj_drc_output")
    },
    ok() {
  ...
  mounted() {
    ipcRenderer.on("obj_drc_dlg", () => {
      this.dialogShow = true
      this.percent = 0
    })
    ipcRenderer.on('drc_input_path_success', (e, paths) => {
      this.options.inputs = paths
    })
    ipcRenderer.on('drc_output_path_success', (e, path) => {
