...
export default {
  methods: {
    openInputPathDlg() {
      ipcRenderer.send("fbx_gltf_input")
    },
    openOutputPathDlg() {
      ipcRenderer.send("fbx_gltf_output")
    },
    ok() {
      ...
      this.percent = 0
      ipcRenderer.send("fbx_gltf_start", _.cloneDeep(this.options))
  ...
  mounted() {
    ipcRenderer.on("fbx_gltf_dlg", () => {
      this.dialogShow = true
      this.percent = 0
    })
    ipcRenderer.on('fbx_input_path_success', (e, paths) => {
      this.options.inputs = paths
    })
    ipcRenderer.on('fbx_output_path_success', (e, path) => {
      this.options.outputPath = path
    })
    ipcRenderer.on('fbx_gltf_progress', (e, percent) => {
